...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/07/04 17:23:03 by caymard           #+#    #+#             */
/*   Updated: 2024/07/05 10:14:14 by caymard          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	get_tex_buffer_img(t_data *data, t_img *img, t_type type)
{
	int	*pixels;
	int	y;
	int	x;
	...
	while (y < img->height)
	{
		...
		while (x < img->width)
		{
			pixels[y * img->width + x] = img->addr[y * img->width + x];
			x++;
		}
		y++;
	}
	data->texture[type].addr = pixels;
	// free(pixels);
	return (0);
}

int	get_img(t_data *data, char *path, int type, t_img *img)
{
...
int	check_texture_file(char *path, char *reader, t_data *data, int type)
{
	...
	close(fd);
	get_img(data, path, type, &data->texture[type]);
	return (0);
}
